...
export function SettingsSidebar({
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('build_sessions').select('id, title, project_type').eq('user_id', session.user.id).order('updated_at', {
        ascending: false
      });
  ...
  return <div className="h-full bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl flex flex-col shadow-lg">
      <div className="flex-shrink-0 border-b border-border/30 flex items-center justify-start px-[24px] py-[9px]">
        <img src="/lovable-uploads/magellan-logo-light.png" alt="Magellan" className="h-12 dark:hidden" />
        <img src="/lovable-uploads/magellan-logo-dark.png" alt="Magellan" className="h-12 hidden dark:block" />
      </div>
      ...
      <ScrollArea className="flex-1">
        <nav className="px-4 space-y-1 py-4">
          ...
          <div className="mb-2">
            <button onClick={() => setIsProjectsOpen(!isProjectsOpen)} className={cn("w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-colors", "text-foreground/80 hover:text-[#03A5C0]", "focus:outline-none")}>
              <span className="flex items-center gap-3">
                {isProjectsOpen ? <ChevronDown className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}
                <span>Projets</span>
              </span>
            </button>
            ...
            {isProjectsOpen && <div className="ml-4 mt-1 space-y-1 border-l border-border/30 pl-3">
              ...
              return <button key={project.id} onClick={() => handleProjectClick(project.id)} className={cn("w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors", isSelected ? "text-[#03A5C0] font-medium" : "text-foreground/70 hover:text-[#03A5C0]", "focus:outline-none truncate")}>
                ...
                <button onClick={handleNewProject} className={cn("w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors mt-2", "text-[#03A5C0] hover:text-[#03A5C0]/80", "focus:outline-none border border-dashed border-[#03A5C0]/50 hover:border-[#03A5C0]")}>
