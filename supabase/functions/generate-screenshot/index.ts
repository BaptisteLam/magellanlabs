...
serve(async (req) => {
  ...
  try {
    ...
    // Use Lovable AI to generate a preview image based on HTML content
    const lovableApiKey = Deno.env.get('LOVABLE_API_KEY')!;
    
    // Extract key info from HTML for AI description
    const titleMatch = htmlContent.match(/<title>(.*?)<\/title>/i);
    const h1Match = htmlContent.match(/<h1[^>]*>(.*?)<\/h1>/i);
    const title = titleMatch ? titleMatch[1] : 'Website';
    const heading = h1Match ? h1Match[1].replace(/<[^>]*>/g, '') : '';
    
    const prompt = `Generate a modern, professional website preview image for: "${title}". ${heading ? `Main heading: "${heading}".` : ''} Make it look like a sleek website screenshot with a clean design, modern UI elements, and an attractive color scheme. 1200x630 aspect ratio.`;

    console.log('Generating screenshot with AI...');
